...
def load_weights(weight_file):
    ...
    try:
        __weights_dict = np.load(weight_file, allow_pickle=True).item()
    except:
        __weights_dict = np.load(weight_file, encoding='bytes', allow_pickle=True).item()

    return __weights_dict
...
class KitModel(nn.Module):
    ...
    def __init__(self, weight_file):
        ...
        '''
        self.conv1 = nn.Conv2d(in_channels=3, out_channels=96, kernel_size=(11, 11), stride=(4, 4), groups=1, bias=True)
        self.conv2 = nn.Conv2d(in_channels=96, out_channels=256, kernel_size=(5, 5), stride=(1, 1), groups=2, bias=True)
        self.conv3 = nn.Conv2d(in_channels=256, out_channels=384, kernel_size=(3, 3), stride=(1, 1), groups=1, bias=True)
        self.conv4 = nn.Conv2d(in_channels=384, out_channels=384, kernel_size=(3, 3), stride=(1, 1), groups=2, bias=True)
        self.conv5 = nn.Conv2d(in_channels=384, out_channels=256, kernel_size=(3, 3), stride=(1, 1), groups=2, bias=True)
        self.fc6_1 = nn.Linear(in_features = 9216, out_features = 4096, bias = True)
        self.fc7_1 = nn.Linear(in_features = 4096, out_features = 4096, bias = True)
        self.f8_chess_1 = nn.Linear(in_features = 4096, out_features = 13, bias = True)
        '''
        ...
        self.fc8_chess_1 = self.__dense(name = 'fc8_chess_1', in_features = 4096, out_features = 13, bias = True)
    ...
    def forward(self, x):
        ...
        fc8_chess_0     = drop7.view(drop7.size(0), -1)
        fc8_chess_1     = self.fc8_chess_1(fc8_chess_0)
        prob            = F.softmax(fc8_chess_1)
        return prob
